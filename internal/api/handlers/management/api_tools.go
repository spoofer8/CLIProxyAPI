...
func (h *Handler) APICall(c *gin.Context) {
	...
	// Match the configured provider's token parameter when testing its endpoint.
	data := body.Data
	if data != "" {
		if compat := helps.OpenAICompatForRequestURL(h.cfg, urlStr); compat != nil {
			model := gjson.Get(data, "model").String()
			if helps.ShouldUseMaxCompletionTokensForModel(compat, model, model) {
				data = string(helps.NormalizeOpenAIMaxTokens([]byte(data), true))
			}
		}
	}
