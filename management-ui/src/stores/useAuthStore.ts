...
const isNamedSession = () =>
  useAuthStore.getState().managementKey === 'cpa-session' &&
  localStorage.getItem('cpa-session-mode') === 'true';

function clearAuthState() {
  const namedSession = isNamedSession();
  ...
  if (namedSession) {
    for (const key of ['cpa-session-mode', 'apiBase', 'apiUrl', 'managementKey']) {
      localStorage.removeItem(key);
    }
  }
  localStorage.removeItem('isLoggedIn');
  if (namedSession) window.location.replace('/login');
}
    ...
    (set, get) => ({
      ...
      logout: () => {
        ...
        logoutPromise = apiClient
          ...
          .then(() => {
            const current = get();
            if (current.apiBase === apiBase && current.managementKey === managementKey) {
              clearAuthState();
            }
          })
          .finally(() => {
...
if (typeof window !== 'undefined') {
  window.addEventListener('unauthorized', () => {
    // An expired session needs only local cleanup, never another server logout request.
    clearAuthState();
  });
