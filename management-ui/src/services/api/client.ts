...
class ApiClient {
  ...
  private requestConnections = new WeakMap<AxiosRequestConfig, number>();
  ...
  private setupInterceptors(): void {
    ...
    this.instance.interceptors.request.use(
      (config) => {
        this.requestConnections.set(config, this.connectionVersion);
        // 设置 baseURL
    ...
    this.instance.interceptors.response.use(
      (response) => {
        // Responses from a previous connection must not alter the active session.
        if (this.requestConnections.get(response.config) !== this.connectionVersion) {
          return response;
        }
  ...
  private handleError(error: unknown): ApiError {
    if (axios.isAxiosError(error)) {
      ...
      if (
        ...
        error.config &&
        this.requestConnections.get(error.config) === this.connectionVersion
      ) {
